...
export const en: Translations = {
  ...
  settings: {
    ...
    gitHub: {
      ...
      loginStarting: 'Starting GitHub login…',
      loginUrlHint: 'Open this URL in your browser and enter the code:',
      ...
      noWorkingFolder: 'No working folder set',
      chooseWorkingFolder: 'Choose folder',
      createRepo: 'Create repository',
      createRepoHint: 'Initialize a git repository in a folder that has none, then use it as the working folder.',
      ...
      resolveConflictsWithAgentPrompt:
        'Resolve whatever is blocking the last sync of this repository. Inspect it with git status first. If conflict markers (<<<<<<<, =======, >>>>>>>) exist in any file: open each conflicted file, decide the correct resolution, apply the edits, and stage the resolved files with git add. If git status shows a merge is still in progress, finish it with git commit --no-edit. If git stash list shows an entry named hermes-sync-autostash: when no merge is in progress, the conflict markers already contain your restored local changes, so drop the redundant entry with git stash drop; when a merge was in progress, restore your local changes with git stash apply, resolve and stage any conflicts it creates, then drop the entry with git stash drop. Keep the current branch commits intact. Report what you changed and why.',
    ...
    gitLab: {
      title: 'GitLab',
      notConnected: 'No GitLab CLI profile connected.',
      connectHint: 'Connect with a personal access token to use GitLab with Hermes.',
      connected: 'Connected',
      logout: 'Sign out',
      loginFailed: 'Could not sign in to GitLab with that token.',
      tokenTitle: 'Connect with a personal access token',
      tokenHint: 'Create a token with the `api` and `read_user` scopes on GitLab, then paste it here.',
      ...
      configGlobal: 'Config Global',
      configLocal: 'Config Local',
      configSetSuccess: 'GitLab account configured',
